...
func auto_estimate_geometry():
	var resolution = tracker.screen_size_lpix
	var size_mm = tracker.screen_size_mm
	
	# Update inputs
	if config_size_w:
		config_size_w.value = size_mm.x
	if config_size_h:
		config_size_h.value = size_mm.y
		
	print("Auto Estimated Screen: Resolution %s px, Phys Size %s mm" % [resolution, size_mm])
...
func _process(delta):
	...
	if tracker.is_face_detected():
		# 1. Head Pose / Nose Gaze projection (viewport-local)
		...
		# 2. Unified Eye Gaze projection (viewport-local)
...
func process_calibration(delta):
	...
	if target_timer >= target_hold_time:
		if tracker.is_face_detected():
			...
			print("OpenCV Space Head Rotation (Pitch, Yaw, Roll deg): Vector3(%f, %f, %f)" % [tracker.get_head_rotation_opencv_space().x, tracker.get_head_rotation_opencv_space().y, tracker.get_head_rotation_opencv_space().z])
			print("OpenCV Space Head Translation (X, Y, Z mm): Vector3(%f, %f, %f)" % [tracker.get_head_translation_opencv_space().x, tracker.get_head_translation_opencv_space().y, tracker.get_head_translation_opencv_space().z])
			print("OpenCV Space Left Eye Center (X, Y, Z mm): Vector3(%f, %f, %f)" % [tracker.get_left_eye_center_opencv_space().x, tracker.get_left_eye_center_opencv_space().y, tracker.get_left_eye_center_opencv_space().z])
			print("OpenCV Space Right Eye Center (X, Y, Z mm): Vector3(%f, %f, %f)" % [tracker.get_right_eye_center_opencv_space().x, tracker.get_right_eye_center_opencv_space().y, tracker.get_right_eye_center_opencv_space().z])
			print("OpenCV Space Gaze Direction (X, Y, Z): Vector3(%f, %f, %f)" % [tracker.get_gaze_direction_opencv_space().x, tracker.get_gaze_direction_opencv_space().y, tracker.get_gaze_direction_opencv_space().z])
...
func process_verification(delta):
	...
	if target_timer > 0.8:
		if tracker.is_face_detected() and eye_gaze_pos != Vector2.ZERO:
			...
			var gaze_screen = tracker.map_logical_to_physical_screen(eye_gaze_pos)
	...
	if target_timer >= target_hold_time:
		...
		if verify_sample_buffer.size() > 0:
			...
			avg_measured = tracker.map_logical_to_physical_screen(eye_gaze_pos)
		...
		var px_size = Vector2(tracker.screen_size_lpix) * tracker.logical_to_physical_pixel_ratio
...
func _draw():
	...
	if draw_report_vectors:
		...
		var window_pos = tracker.map_logical_to_physical_screen(Vector2.ZERO)
		for res in verify_results:
			var target_win = (res.target - window_pos) / tracker.logical_to_physical_pixel_ratio
			var measured_win = (res.measured - window_pos) / tracker.logical_to_physical_pixel_ratio
