...
class SelectControl:
	...
	func _init(title, val, choices, hint=""):
		...
		var select_idx = 0
		var choices_arr = choices.keys() if typeof(choices) == TYPE_DICTIONARY else choices
		for i in range(choices_arr.size()):
			var item_str = str(choices_arr[i])
			value_ctrl.add_item(item_str)
			if(val == choices_arr[i] or val == item_str):
				select_idx = i
