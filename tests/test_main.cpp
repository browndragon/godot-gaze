...
{
    GodotCameraVector3 a(1.0, 2.0, 3.0);
    GodotCameraVector3 b(4.0, 5.0, 6.0);

    GodotCameraVector3 c = a + b;
    CHECK(c.x == doctest::Approx(5.0));
    CHECK(c.y == doctest::Approx(7.0));
    CHECK(c.z == doctest::Approx(9.0));

    GodotCameraVector3 d = b - a;
...
TEST_CASE("Testing Projection Engine Math (Zero Tilt)")
{
    ...
    engine.set_screen_size_pixels(GodotDisplayVector2(1920.0, 1080.0));
    engine.set_screen_size_mm(SpacedVector2<Space::GodotDisplayMm>(527.0, 296.0));

    // Camera placed top-center, 10mm in front of screen plane (offset relative to top bezel)
    CameraPlacement placement(GodotCameraVector3(0.0, 0.0, 10.0), 0.0);
    ...
    GodotCameraVector3 dir(0.0, 0.0, 1.0); // Looking straight at camera on top bezel
...
TEST_CASE("Testing Projection Sensitivity under Retina Dimensions")
{
    ...
    engine.set_screen_size_pixels(GodotDisplayVector2(1440.0, 900.0));
    engine.set_screen_size_mm(SpacedVector2<Space::GodotDisplayMm>(166.0, 103.0));

    CameraPlacement placement(GodotCameraVector3(0.0, 0.0, 0.0), 0.0);
    engine.set_camera_placement(placement);

    GodotCameraVector3 origin(0.0, 0.0, -500.0);
    GodotCameraVector3 dir(-0.1, 0.0, 1.0); // 5.7 degrees right rotation (-X in camera space = screen right)
...
TEST_CASE("Testing Projection Engine Math (With Tilt)")
{
    ...
    CameraPlacement placement(GodotCameraVector3(0.0, 0.0, 10.0), 15.0);
    engine.set_camera_placement(placement);

    GodotCameraVector3 origin(0.0, 0.0, -500.0);
    GodotCameraVector3 dir(0.0, 0.0, 1.0); // Looking straight along camera optical axis
...
TEST_CASE("Testing Monotonicity and Calibration Mappings from User Logs")
{
    ...
    auto get_unmirrored_forward = [](double pitch_deg, double yaw_deg, double roll_deg)
    {
        ...
        return GodotCameraVector3(fx / len, fy / len, fz / len);
    };
    ...
    GodotCameraVector3 f1 = get_unmirrored_forward(p1_pitch, p1_yaw, p1_roll);
    GodotCameraVector3 f2 = get_unmirrored_forward(p2_pitch, p2_yaw, p2_roll);
    GodotCameraVector3 f3 = get_unmirrored_forward(p3_pitch, p3_yaw, p3_roll);
    GodotCameraVector3 f4 = get_unmirrored_forward(p4_pitch, p4_yaw, p4_roll);
...
TEST_CASE("TDD: Thorough physical verification of Camera-to-Screen Transform & Projection cases")
{
    struct TransformTestScenario
    {
        ...
        GodotCameraVector3 camera_offset_mm;
        GodotDisplayVector2 screen_size_px;
        SpacedVector2<Space::GodotDisplayMm> screen_size_mm;
        GodotCameraVector3 head_position_cam;
        GodotDisplayVector2 target_screen_px;
    };
    ...
    std::vector<TransformTestScenario> scenarios = {
        // 1. Laptop: Seated Center, Looking at Screen Center (15 degree screen lean, top center bezel camera)
        {
            ...
            15.0,
            GodotCameraVector3(0.0, 0.0, 0.0),
            GodotDisplayVector2(1920.0, 1080.0),
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(12.2, 0.3, -739.5),
        ...
        // 3. Laptop: Seated Center, Looking at Screen Bottom-Edge
        {
            ...
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(12.2, 0.3, -739.5),
            GodotDisplayVector2(960.0, 1080.0)},
        ...
        // 5. Laptop: Seated Center, Looking at Screen Right-Edge
        {
            ...
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(12.2, 0.3, -739.5),
            GodotDisplayVector2(1920.0, 540.0)},
        // 6. Laptop: Seated Left, Looking at Bottom-Right
        {
            ...
            GodotDisplayVector2(1920.0, 1080.0),
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(46.9, 4.5, -775.6),
            GodotDisplayVector2(1920.0, 1080.0)},
        // 7. Laptop: Seated Right, Looking at Top-Left
        {
            ...
            GodotDisplayVector2(1920.0, 1080.0),
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(-14.2, -1.1, -725.5),
            GodotDisplayVector2(0.0, 0.0)},
        // 8. Laptop: Looking off-screen Left
        {
            ...
            SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0),
            GodotCameraVector3(12.2, 0.3, -739.5),
            GodotDisplayVector2(-100.0, 540.0)}};
    ...
    for (const auto &sc : scenarios)
    {
        ...
        GodotCameraVector3 P_cam_target(x_cam, y_cam, z_cam);

        // 2. Verify Ray Intersection in ProjectionEngine
        GodotCameraVector3 V_cam = (P_cam_target - sc.head_position_cam).normalized();
        ...
        GodotCameraVector3 basis_col0(-scale_x, 0.0, 0.0);
        GodotCameraVector3 basis_col1(0.0, -cos_t * scale_y, sin_t);
        GodotCameraVector3 basis_col2(0.0, sin_t * scale_y, cos_t);
        GodotCameraVector3 translation(
...
TEST_CASE("Testing Gaze Projection Invariance and Monotonicity (Yaw Sweep)")
{
    ...
    GodotCameraVector3 camera_offsets[] = {
        GodotCameraVector3(0.0, 148.0, 10.0),  // Top center
        GodotCameraVector3(0.0, -148.0, 10.0), // Bottom center
        GodotCameraVector3(-250.0, 0.0, 10.0), // Left side
        GodotCameraVector3(250.0, 0.0, 10.0)   // Right side
    };
    ...
    for (double tilt : tilts)
    {
        for (const auto &offset : camera_offsets)
        {
            ...
            for (int i = 0; i <= 20; ++i)
            {
                ...
                GodotCameraVector3 dir(vx, 0.0, vz);
    ...
    for (double tilt : tilts)
    {
        for (const auto &offset : camera_offsets)
        {
            ...
            for (int i = 0; i <= 20; ++i)
            {
                ...
                GodotCameraVector3 dir(0.0, vy, vz);
...
TEST_CASE("Testing Gaze Projection Invariance (Head Translation Sweep)")
{
    ...
    CameraPlacement placement(GodotCameraVector3(0.0, 148.0, 10.0), 0.0);
    engine.set_camera_placement(placement);

    // Gaze direction is straight forward
    GodotCameraVector3 dir(0.0, 0.0, 1.0);
    ...
    for (int i = 0; i <= 20; ++i)
    {
        ...
        GodotCameraVector3 origin(ox, 0.0, -500.0);
    ...
    for (int i = 0; i <= 20; ++i)
    {
        ...
        GodotCameraVector3 origin(0.0, oy, -500.0);
...
TEST_CASE("Testing High-DPI and Logical/Physical Coordinate Transformations")
{
    ...
    engine.set_screen_size_pixels(GodotDisplayVector2(1920.0, 1080.0));
    engine.set_screen_size_mm(SpacedVector2<Space::GodotDisplayMm>(305.0, 191.0));
    ...
    for (double dpr : dprs)
    {
        ...
        GodotDisplayVector2 logical_pixel(physical_pixel.x / dpr, physical_pixel.y / dpr);

        // Logical screen size is screen_size_pixels / dpr
        GodotDisplayVector2 screen_size_logical(1920.0 / dpr, 1080.0 / dpr);
