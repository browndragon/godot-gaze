...
TEST_CASE("ORT YuNet 5-Keypoint Extraction on Real Image self_center.jpg")
{
    ...
    // Lock exact numerical ground-truth keypoint values for self_center.jpg under Center-Crop
    ...
    CHECK(res.head_pose.pitch_rad == doctest::Approx(-0.217).epsilon(0.02));
    CHECK(res.head_pose.yaw_rad == doctest::Approx(0.147).epsilon(0.02));
