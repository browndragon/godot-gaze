...
// TODO: This, and the `run_sidecar_tests.js` and `run_web_tests.js`, seem to be in the wrong place to me? Are these just test files? What are they doing here? Shouldn't they be in tests/ or tests/web or something?
...
const PORT = 9225; // Chrome debugger port
const SERVER_PORT = 8210; // test server port
// TODO: Will this work crossplatform? What's a more standard way of doing this? Probably needs injecting.
const CHROME_PATH =
...
const server = http.createServer((req, res) => {
  ...
  // TODO: Does this NEED SharedArrayBuffer? I think we don't usually use them, only in the threaded web builds (which we disprefer).
...
// 2. Launch Chrome
// TODO: Is there standard testing infrastructure we can use here? puppeteer? IDK.
...
const safetyTimeout = setTimeout(() => {
  console.error(
    "[TestRunner] Error: Safety timeout reached. Headless tests took too long and were aborted.",
  );
  try {
    chromeProcess.kill();
  } catch (e) {}
...
async function run() {
  ...
  try {
    ...
    // TODO: IMO everything above here was infrastructure-y test setup. PLEASE extract it standalone, and/or replace with standard testing infrastructure?, rather than regurgitating inline! It makes the test very hard to read!

    // Wait for page load and model initialization
    console.log(
      "[TestRunner] Waiting for page and models to initialize (approx 4s)...",
    );
    await sleep(4000);
    ...
    for (const imgName of testImages) {
      ...
      console.log(
        `  Left Eye Center:  (${data.lex.toFixed(2)}, ${data.ley.toFixed(2)}, ${data.lez.toFixed(2)})`,
      );
      console.log(
        `  Right Eye Center: (${data.rex.toFixed(2)}, ${data.rey.toFixed(2)}, ${data.rez.toFixed(2)})`,
      );
      console.log(
        `  Gaze Dir:         (${data.dx.toFixed(4)}, ${data.dy.toFixed(4)}, ${data.dz.toFixed(4)})`,
      );
      console.log(
        `  Head Translation: (${data.tx.toFixed(2)}, ${data.ty.toFixed(2)}, ${data.tz.toFixed(2)})`,
      );
      console.log(
        `  Head Rotation:    (${data.rx.toFixed(4)}, ${data.ry.toFixed(4)}, ${data.rz.toFixed(4)})`,
      );
      ...
      console.log(
        `  [Parity Check] Left Eye Diff:  (${diffLex.toFixed(4)}, ${diffLey.toFixed(4)}, ${diffLez.toFixed(4)})`,
      );
      console.log(
        `  [Parity Check] Right Eye Diff: (${diffRex.toFixed(4)}, ${diffRey.toFixed(4)}, ${diffRez.toFixed(4)})`,
      );
      console.log(
        `  [Parity Check] Gaze Dir Diff:  (${diffDx.toFixed(4)}, ${diffDy.toFixed(4)}, ${diffDz.toFixed(4)})`,
      );
      console.log();
      ...
      if (
        ...
        throw new Error(
          `Math Parity Assertion Failed for ${imgName}! Diff exceeded tolerance of ${tol}.`,
        );
      }
    }

    // TODO: Please at least make each test its own function, to aid file navigation. Again, using testing modules/libs for this would make it a lot easier to navigate.
    ...
    const { windowId } = await sendCommand("Browser.getWindowForTarget", {
      targetId,
    });
    ...
    async function getCanvasGazeCoords() {
      ...
      if (res.exceptionDetails) {
        throw new Error(
          `Window test frame failed: ${res.exceptionDetails.exception.description}`,
        );
      }
      return {
    ...
    console.log(
      `  At (100, 200): canvasX=${coords1.canvasX}, canvasY=${coords1.canvasY}`,
    );
    ...
    console.log(
      `  At (250, 100): canvasX=${coords2.canvasX}, canvasY=${coords2.canvasY}`,
    );
    ...
    console.log(
      `  Asserting X delta: Expected ${deltaWinLeft}, Got ${deltaCanvasX}`,
    );
    console.log(
      `  Asserting Y delta: Expected ${deltaWinTop}, Got ${deltaCanvasY}`,
    );

    if (Math.abs(deltaCanvasX - deltaWinLeft) > 2) {
      throw new Error(
        `Assertion Failed: canvasX delta (${deltaCanvasX}) does not match window displacement (${deltaWinLeft})`,
      );
    }
    if (Math.abs(deltaCanvasY - deltaWinTop) > 2) {
      throw new Error(
        `Assertion Failed: canvasY delta (${deltaCanvasY}) does not match window displacement (${deltaWinTop})`,
      );
    }
    ...
    console.log(
      `  At 1024x768: canvasX=${coords3.canvasX}, canvasY=${coords3.canvasY}`,
    );

    if (
      Math.abs(coords3.canvasX - coords2.canvasX) > 2 ||
      Math.abs(coords3.canvasY - coords2.canvasY) > 2
    ) {
      throw new Error(
        `Assertion Failed: Canvas screen coordinates shifted during window resize: coords2=(${coords2.canvasX}, ${coords2.canvasY}), coords3=(${coords3.canvasX}, ${coords3.canvasY})`,
      );
    }
    console.log(
      "  Canvas screen coordinates remained invariant to window resizing.",
    );

    // Test Case 6: Dynamic Device Pixel Ratio (DPR) Scaling Stability
    console.log("\n--- Running Dynamic DPR Coordinate Scaling Tests ---");
    console.log(
      "[TestRunner] Test Case 6: Dynamic Device Pixel Ratio (DPR) Scaling Stability",
    );
    ...
    for (const dprVal of dprValues) {
      ...
      console.log(
        `    At DPR=${dprVal}: canvasX=${coords.canvasX}, canvasY=${coords.canvasY}`,
      );
      ...
      if (
        Math.abs(coords.canvasX - expectedX) > 2 ||
        Math.abs(coords.canvasY - expectedY) > 2
      ) {
        throw new Error(
          `Assertion Failed: Coordinates at DPR=${dprVal} (${coords.canvasX}, ${coords.canvasY}) do not match expected scaling (${expectedX}, ${expectedY})`,
        );
      }
    }
    console.log(
      "  Canvas screen coordinates scaled linearly and correctly with changing devicePixelRatio.",
    );
    ...
    console.log(
      "  Injecting window.screenX/screenLeft security error simulation...",
    );
    await sendCommand("Runtime.evaluate", {
      expression: `
    ...
    console.log(
      `  Simulated sandbox windowed coordinates: canvasX=${sandboxCoords.canvasX}, canvasY=${sandboxCoords.canvasY}`,
    );
    ...
    console.log(
      `  Simulated sandbox fullscreen coordinates: canvasX=${fullscreenCoords.canvasX}, canvasY=${fullscreenCoords.canvasY}`,
    );
    ...
    console.log(
      "[TestRunner] Window relocation, resizing, and sandbox tests passed!",
    );
    ...
    for (const imgName of testImages) {
      if (!results[imgName].face_detected) {
        throw new Error(
          `Assertion Failed: Face should be detected in ${imgName}`,
        );
      }
    }
    ...
    for (const imgName of testImages) {
      ...
      if (data.lex <= data.rex) {
        throw new Error(
          `Assertion Failed: Left eye X should be greater than right eye X in ${imgName}. lex=${data.lex}, rex=${data.rex}`,
        );
      }
    }
    ...
    console.log(
      `  Verifying translation monotonicity: left.tx (${left.tx.toFixed(2)}) > right.tx (${right.tx.toFixed(2)})`,
    );
    if (left.tx <= right.tx) {
      throw new Error(
        `Assertion Failed: Translation X monotonicity failed. left=${left.tx}, right=${right.tx}`,
      );
    }

    // Gaze direction monotonicity: left.dx > right.dx
    console.log(
      `  Verifying gaze direction monotonicity: left.dx (${left.dx.toFixed(4)}) > right.dx (${right.dx.toFixed(4)})`,
    );
    if (left.dx <= right.dx) {
      throw new Error(
        `Assertion Failed: Gaze direction X monotonicity failed. left=${left.dx}, right=${right.dx}`,
      );
    }
