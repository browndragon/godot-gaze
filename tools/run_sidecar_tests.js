...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === "/tests/test_sidecar.html") {
    ...
    res.end(
      fs.readFileSync(path.join(__dirname, "../tests/test_sidecar.html")),
    );
  } else if (urlPath === "/src/web/gaze_sidecar.js") {
    res.setHeader("Content-Type", "application/javascript");
    res.end(
      fs.readFileSync(path.join(__dirname, "../src/web/gaze_sidecar.js")),
    );
...
async function run() {
  ...
  try {
    ...
    await sendCommand("Runtime.evaluate", {
      expression: "window.lastLoadedMethod = undefined;",
    });
    ...
    if (!result1.success || result1.method !== "local") {
      throw new Error(
        `Test 1 Failed: Expected success with method 'local', got: ${JSON.stringify(result1)}`,
      );
    }
    console.log("Test 1 PASSED!\n");

    console.log(
      "--- Running Test 2: Local fetch fails, falls back to CDN script ---",
    );
    // Reset state
    await sendCommand("Runtime.evaluate", {
      expression: "window.lastLoadedMethod = undefined;",
    ...
    if (!result2.success || result2.method !== "cdn") {
      throw new Error(
        `Test 2 Failed: Expected success with method 'cdn' via fallback, got: ${JSON.stringify(result2)}`,
      );
    }
